...
#print(gpt3_completion('quel jour on est?'))
...
def ask_question_to_pdf(question):
    return(gpt3_completion(question + document + "Tu répondras à cette question en t'appuyant uniquement sur le texte précédent."))
...
def gpt3_question():
    question_cree = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": document + "Pose-moi une question sur ce texte."}
        ]
    )
...
def gpt3_answer(reponse):
    vrai_ou_faux = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": document + "en te basant uniquement sur ce texte, est-ce que" + reponse + "répond à la question" + la_question + "? Tu répondras vrai si la réponse est vraie et faux si elle est fausse, en argumentant."}
        ]
    )
